...
export default function RankingPage() {
  ...
  return (
    <main className="min-h-screen bg-[#1f1f1f] px-8 py-8 text-white">
      <div className="mx-auto max-w-7xl">
        <header className="mb-10 flex items-center justify-between">
          ...
          <a
            href="/"
            className="border border-white/30 px-5 py-3 text-sm font-semibold transition hover:bg-white hover:text-black"
          >
            Home
          </a>
        </header>

        <h1 className="mb-8 text-5xl font-black">Anime Tier Ranking</h1>
        ...
        <div className="border border-black">
          {tiers.map((tier) => (
            ...
            >
              ...
              <div className="flex flex-1 flex-wrap gap-3 bg-[#111] p-4">
                {tierItems[tier.name].map((item) => (
                  <div
                    key={item}
                    draggable
                    onDragStart={() => setDraggedItem(item)}
                    className="cursor-grab border border-white/20 bg-black px-5 py-4 font-semibold"
                  >
                    {item}
                  </div>
                ))}
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          {items.map((item) => (
            <div
              ...
              draggable
              onDragStart={() => setDraggedItem(item)}
              className="cursor-grab border border-white/20 bg-black px-5 py-4 font-semibold transition hover:bg-white hover:text-black"
            >
              {item}
            </div>
          ))}
          ...
          <div className="mt-8 space-y-3">
            {results.length === 0 ? (
              ...
              results.map((result, index) => (
                <div
                  key={result.item}
                  className="flex items-center justify-between border border-white/20 bg-black p-5"
                >
